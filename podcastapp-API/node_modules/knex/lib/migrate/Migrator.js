...
class Migrator {
  ...
  rollback(config, all = false) {
    ...
    return _bluebird.default.try(() => {
      this.config = getMergedConfig(config, this.config);
      return migrationListResolver.listAllAndCompleted(this.config, this.knex).tap(value => validateMigrationList(this.config.migrationSource, value)).then(val => {
        const _val = _slicedToArray(val, 2),
              allMigrations = _val[0],
              completedMigrations = _val[1];
